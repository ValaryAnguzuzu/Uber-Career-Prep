...
#we can simply use a set, removed dups for us
#but this is not in place, its also O(n) time

#we can use two pointers to solve this in O(n) time and O(1) space


def removeDuplicates(nums):
    """
    TC: O(N) - we are iterating thru the array once
    SC: O(1) - we are doing this in place
    """
    if not nums:
        return 0
    l = 1 #keeps track of psns to replace
